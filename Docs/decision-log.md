...
### 2026-09-25 — Testing<br>
**Question:** Did the DC-8/DC-9 fix (commit `749ed14`) and the two replacement test-data URLs (RU-5/RU-6, DC-6) hold up live once deployed (run #33)?<br>
**Decision:** Yes, all three confirmed against the real Discogs API. `release/32391060` (Brotherhood, now doubling as both RU-5/RU-6 and DC-5/DC-8/DC-9's example) now breaks down as Side A, Side B, CD1, CD2, DVD1, DVD2 — correctly numbered since this release actually has 2 CDs, not 1 as assumed during implementation, which incidentally confirms the singular/plural detection logic handles the plural CD case as well as the plural DVD case. Grand total unchanged at `06:18:04`, confirming the fix is a grouping/labeling change only, not a totals change. `release/1040301` (the new Kate Bush DC-6 URL, an 8xCD box set) now successfully exercises the box-set/heading duration logic for the first time — 8 "Item" groups, grand total `06:06:27` — closing out the DC-6 test case that the old master-URL had blocked.<br>
**Reasoning:** Closing the loop on both the DC-8/DC-9 requirement and the two test-data-doc gaps flagged earlier (DC-6's master URL, RU-5/RU-6's untimed vinyl tracks) in the same pass, since the same release now serves double duty for RU-5/RU-6 and DC-5/DC-8/DC-9.<br>

### 2026-09-26 — Implementation / Bug fix<br>
**Question:** During Bruce's first manual pass over the deployed MVP, the error banner (empty, red, warning icon) appeared both on page load and after a successful calculation — not just on an actual error. Why?<br>
**Decision:** CSS cascade bug, not a JS logic bug. `errorBanner.hidden` was being set correctly by `app.js` throughout (`true` on load, `true` after a successful calculation, `false` only on a real error), but `.error-banner` in `styles.css` had an unconditional `display: flex` rule with the same specificity as the browser's built-in `[hidden]{display:none}` rule — and, being an author-stylesheet rule, it won the cascade regardless of the `hidden` attribute. Fixed by adding an explicit `.error-banner[hidden]{display:none}` override (commit `130a3b5`). Checked for the same pattern elsewhere: `#results` is the only other element toggled via `.hidden` in `app.js`, and it has no competing `display` rule of its own (only its child `.results-grid` does), so it was never affected.<br>
**Reasoning:** A styling/cascade bug rather than a computation bug, caught by Bruce's own manual testing pass rather than the live-testing pass done earlier in the project (which focused on calculation correctness, not visual/UI states) — a reminder that functional live-testing and manual UI review catch different classes of bugs.<br>

### 2026-09-26 — Requirements / Implementation
**Question:** Bruce updated the Figma mockup and PRD with two changes: fold the app name into the header (UI-2 — drop the separate "Discogs database tool" eyebrow line, make the h1/title just "Discogs Release Duration Calculator"), and clarify release-date formatting (new RU-9). How should each be implemented?<br>
**Decision:** UI-2: removed the `.eyebrow` div from `index.html` and its now-unused CSS rule from `styles.css`; the `<h1>` and `<title>` now read "Discogs Release Duration Calculator" directly. RU-9: added `formatReleaseDate()` in `app.js`, handling the three shapes Discogs' `released` field actually returns (confirmed against the project's own Postman/OpenAPI fixture, e.g. `"2005-05-24"`): a full date → `Mon D, YYYY` (e.g. `Nov 10, 1978`), a year-only value → `YYYY`, and a month-and-day-only value → `Mon D`; any other shape (e.g. Discogs' literal `"Unknown"` text) passes through unchanged rather than being hidden or mis-parsed. Bruce also replaced `Docs/figma-concepts/release-duration-calculator-concept.png` with the updated mockup directly on his machine, since transferring a binary image asset from this cloud session turned out to be far slower and riskier than having him place the file himself (see reasoning below). All changes committed together as `bbfe6f4`, verified against the local Node test harness (46/46 passing, including 6 new RU-9 cases) before commit.<br>
**Reasoning:** Both were small, well-scoped PRD/Figma-driven changes with no ambiguity once the updated docs were read directly. Separately: an attempt to transfer the new Figma PNG into the repo via this session (base64-encoding the image, paginating it through the assistant's own context, then reconstructing it in a device shell command) proved far more expensive in time and risk than the entire initial build of the app — repeated compression passes were needed just to make the payload transferable, with real risk of silent transcription corruption along the way. Bruce flagged this directly ("this minor change was taking longer than the full initial build of the app"). Going forward, any binary asset (image, PDF, audio, etc.) that needs to land in the repo on Bruce's machine will default to Bruce placing the file himself, rather than attempting an in-context transfer — that channel is appropriate only for small text files.<br>

### 2026-09-26 — Requirements / Implementation
**Question:** Bruce wanted a way to confirm which deployed build he (or a friend) is actually looking at, given the earlier error-banner fix needed a hard refresh before it was visible — should the app display a version number?<br>
