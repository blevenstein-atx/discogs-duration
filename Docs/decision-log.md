...
### 2026-09-09 — Tooling
**Question:** Install Homebrew, Node.js, and GitHub CLI locally, even though a static JS site doesn't strictly require Node.js to run in the browser?
...
**Question:** Register a formal Discogs "Application" (full OAuth client), or is a personal access token enough?
...
**Decision:** No to all three for MVP — release URL/ID entry only, public data only, no AWS. All three deferred (search and OAuth to Future, AWS to 2.0).
...
**Question:** How should scope be organized so MVP architecture choices can be made with extensibility in mind, without pulling later-phase features into the MVP itself?
...
### 2026-09-13 — Tooling
**Question:** What format for the API documentation — a Postman-native "v3 YAML" format, or something else?
...
### 2026-09-13 — Tooling
**Question:** Postman's file-based workspace sync generates a `postman/` folder plus a hidden `.postman/` local-state folder — what gets committed to the repo?
**Decision:** Commit `postman/specs`, `postman/collections`, and `postman/globals`; gitignore `.postman/`.
...
**Question:** Should the full MVP/2.0/Future scope be written and reviewed by Bruce before Claude makes implementation/extensibility decisions?
...
**Decision:** As plain text/markdown in the GitHub repo (`Docs/`, `postman/`), not in Apple Notes or Claude's own Project docs.
...
**Question:** For the 2.0 architecture, is the intended split "Vercel/Netlify hosts the frontend, AWS hosts the backend/API" — two complementary pieces, not competing choices?
...
**Question:** GitHub's own UI prompts to add a README — add one now, or wait until the project's finished as originally planned?
**Decision:** Add a short interim README now (what the app does, current status, links to scope/PRD); keep the full architecture-and-decisions README deferred to project completion.
...
**Question:** How to get genuine hands-on exposure to "AI-assisted workflow" / "AI prototyping" tooling (increasingly named in PM job postings) without diluting this project's own scope?
...
**Question:** Should Lovable be used to prototype the (currently deferred) Discogs Search feature before committing it to the real roadmap?
**Decision:** Yes — added "Use Lovable to prototype Discogs Search POC" to the v2 section of `scope.md`, explicitly worded as a spike, not a scope commitment.
...
**Decision:** Not yet — a full requirements review was performed before any build work, per the standing "no building before requirements review" agreement. Rated 8/10: strong structure and edge-case instinct, docked for one real logical conflict and a handful of gaps (see Open Questions below).
...
**Decision:** Yes — spelling/typo pass committed, no content changes except restoring one accidentally-dropped word ("does **not** calculate...") and fixing a skipped use-case step number.
