...
# Problems
Discogs release data often inlcudes track times, but does calculate and display total release durartion.
Summing track times in your head, or using a calculator or spreadhseet is onerous and time-consuming. Asking an AI agent for the duration is non-deterministic and prone to error and variation.
...
Not every disocgs release has complete track time data.

# Use Cases
Keeping these intentioanlly simple and lightweight, no UML or mermaid diagrams or strcutured use case templates.


## Happy Path

1. User enters or pastes a discogs URL or release ID.
2. App succesfully validates the input.
...
2. App succesfully validates the input.
3. User clicks to initiate the duration calculation.
4. App pulls release data but track durartions are missing, incomplete, or in an unsupported pattern.
6. App displays error message.
7. User may clear results and request a new URL or release ID.
...
2. App succesfully validates the input.
3. User clicks to initiate the duration calculation.
4. App is unable to connect to the discogs API (i.e. 500 serires error).
5. App displays error message.
6. User may try again, resubmitting the same release URL or ID.

# Input

Ability to enter a discogs release URL. Teh release ID can be parsed from the URL.
Ability to enter a discogs release ID.

## Input Validation

A URL must be in a valid disocgs URL format. Example: https://www.discogs.com/release/547049-Gorillaz-Demon-Days, or www.discogs.com/release/547049-Gorillaz-Demon-Days.
...
Disallow use of non-release discogs records. Wrong Discogs URL type: a /master/ URL instead of /release/ (easy mistake — masters and releases are different endpoints with different data), or a /artist/, /label/, or search-results URL. This can be identified at time of input, such as "master" in the URL, like as https://www.discogs.com/master/58002-Gorillaz-Demon-Days. Or by a leading m in ID, such as [m58002] or m58002. In the case of a user entering just a numeric ID, it will not be possible to detect a master release until the API response is received.
If a non-release URL or master release ID with leading m is entered, prior to API request display a message: Only indivisual releases are supported. Please use a specfiic release URL or ID.
...
Beneath the app name and above the input field place instructions: Enter a discogs release URL or disocgs release ID to calculate the release's total duration. Master release records are not supported, only specific child releases.
A single input field for entering either a release URL or release ID. Field label: Release URL or ID
Make the inout field long enough to limit trucation of an entered URL.
...
As the input and output is minimal, let's keep both the input and output on the same screen. The results can frender beneath the input field. This has the advantage of also keeping the input context visble to the user.
In all success and error cases, the inout URL or ID should remain in the input field. The user has the abiluty to edit or replace the input and try again.
The results should have two sections: Release metadata, and the duration calculation
Release metada to include: Discogs release ID, Artist, Title, Label, Format, Country, Released date
...
In the case of a formats with multiple sides (vinly LPs, casssettes) indicated for example as A, B, or AA, include sub-total durations for the sides as: Side Duration: hh:mm:ss
...
In the case of releases with a mutli-track medly/suite, these will appear in the track list with a medly/suite section header and indented track, such as: https://www.discogs.com/release/4000806-Genesis-The-Lamb-Lies-Down-On-Broadway. The section header may have a duration, while the contained tracks do not. If the contained tracks have times, ignore the header and simply treat these as regular tracks in the total durartion. If the contained tracks have no individual times, use the time in the medly/suite header.
In the case of release with section headers not related to a medly/suite, such as section names or Bonus Tracks, ignore the headers. For example: https://www.discogs.com/release/1141075-Kate-Bush-Hounds-Of-Love
...
In the case of no track times or missing track times for one or more tracks in a single or multi-disc standard relase (double LP or CD, for example), do not calculate duration. Display a message: Duration cannot be calculated due to missing track time data.
...
In the case of any other 400 errors, 500 errors, timeouts, or unexepcted malformed API respones errors, display an error message: Something went wrong. Inlcude any returned error code or message from disogs beneath the error message/
